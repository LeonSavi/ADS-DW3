...
def plot_overfitting(concatenated: pd.DataFrame, k:int=8):
    """
    Plots training versus test RMSE for each model to visually assess overfitting. 
    Points near the dashed diagonal (Train RMSE = Test RMSE) indicate good generalization, 
    while points above = overfitting.
    """
    plt.figure(figsize=(6,5))
    sns.scatterplot(
        data=concatenated,
...
```
In the codes below we find the best hyperparameters without re-running the whole optuna study.
